...
1. **Стресс-тест пройден:** стенд с --tearing --recreate (пересоздание свопчейна в рантайме) + инъекция + лимит; резкие смены 60→30→выкл→40 применяются чисто; после всех изменений лимит 40 FPS держится (39.3–41.0 FPS, p50 30.6ms, p99 32.4ms); процесс жив; 1151 present за прогон. Пересоздание свопчейна подхватывается хуками фабрики без ручного вмешательства (M4-механизм работает).

2. **Сравнение с RTSS задокументировано в README:** архитектурная разница — RTSS держит УЖЕ ГОТОВЫЙ кадр до целевого времени (+ до кадра задержки), iFrame спит ПОСЛЕ Present, задерживая только старт следующего кадра (+0 задержки). GPU не простаивает, кадр уходит в момент готовности.

3. **Релиз v1.0.0:** README (идея, фичи, использование, сборка, архитектура, ограничения), версия поднята 0.1.0 → 1.0.0, финальные сборки x64 (app + DLL) и x86 (DLL) чистые, 13/13 тестов.
...
- [x] M7 — стресс-тест ✅ (tearing + recreate + резкие смены лимита 60→30→выкл→40: лимит держится 39.3–41.0 FPS, p50 30.6ms, процесс жив, 1151 present); сравнение с RTSS ✅ (README: JIT-wait vs hold-model, +0 задержки); релиз v1.0.0 ✅ (README, финальные сборки x64+x86, 13/13 тестов)